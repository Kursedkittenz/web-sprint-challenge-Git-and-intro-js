...
function getArtistByIndex(myArr, myIndex) {
  return `the artist at index ${myArr[myIndex]['id']} is ${myArr[myIndex]['name']}`
}
...
function listOfNames(myArr) {
  let newArr = myArr.splice(0)
  for(const x in newArr){
    newArr[x] = newArr[x]['name']
  }
  return newArr
}
...
function removeArtist(myArr,myIndex) {
  let newArr = myArr.splice(0)
...
2. Recieve a string (name), a string (years), a string (genre), a string (nationality), and a string (bio) as 
arguments passed from the SECOND, THIRD, FOURTH, FIFTH, and SIXTH parameters, repsectively.
...
function addArtist(myArr, myName, myYears, myGenre, myNationality, myBio) {
  myArr.push({name: myName, years: myYears, genre: myGenre, nationality: myNationality, bio: myBio})
  return myArr
}
...
function lotsOfArt(myArr) {
  let newArr = []
  for(const x in myArr){
    if(myArr[x]['paintings'] > 100){
      newArr.push(myArr[x]['name'])
    }
  }
  return newArr
}
...
function artistInfo(myArr, myName){
  for(const x in myArr){
...
function artistByCountry(myArr, myNat){
  let newArr = []
  for(const x in myArr){
    if(myArr[x]['nationality'] === myNat){
      newArr.push(myArr[x]['name'])
    }
  }
  return newArr
